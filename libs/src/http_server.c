...
int HTTPServer_Initiate(HTTPServer* _Server, HTTPServer_OnConnection _OnConnection, int _backlog)
{
	_Server->onConnection = _OnConnection;

	TCP_server_init(&_Server->tcpServer, "58080", HTTPServer_OnAccept, int _backlog);
	
	_Server->task = smw_create_task(_Server, HTTPServer_TaskWork);
