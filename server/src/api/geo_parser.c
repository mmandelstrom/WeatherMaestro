...
int geo_parser_parse_nominatim_geo(Geos* _Geos, Nominatim_Geo* _NOM_Geos, int _geo_count);
...
int geo_parser_get_geo_from_api_by_coords(Geos* _Geos, ExternalGeoAPI _ExtAPI)
{
  ...
  else if (_ExtAPI == BIGDATACLOUD_GEO)
  {
    ...
    result = bigdatacloud_get_geo_by_coords(BDC_Geo, _Geos->geo[0]->lat, _Geos->geo[0]->lon);
    if (result != 0)
    {
...
int geo_parser_parse_nominatim_geo(Geos* _Geos, Nominatim_Geo* _NOM_Geo, int _geo_count)
{
  ...
  {
    if (i < _Geos->count) // Make sure we don't try to parse more than we have allocated
    {
...
char* geo_parser_get_cache_filepath(Geos* _Geos)
{
  ...
  else if (_Geos->geo[0] != NULL) // use lat+lon for filename via CACHE_FILENAME_LOCATION template
  {
...
/* int geo_parser_get_geo_from_cache(Geos* _Geos, const char* _filepath)
{
  ...
  for (int i = 0; i < geo_count; i++)
  {
    ...
    if (Json_Geo != NULL && i < _Geos->count)
    {
      ...
      _Geos->geo[i]->country        = strdup(json_get_string(Json_Geo, "country")); 
      _Geos->geo[i]->city           = strdup(json_get_string(Json_Geo, "city")); 
      _Geos->geo[i]->county         = strdup(json_get_string(Json_Geo, "county")); 
      _Geos->geo[i]->street         = strdup(json_get_string(Json_Geo, "street")); 
      _Geos->geo[i]->postcode       = strdup(json_get_string(Json_Geo, "postcode")); 
      _Geos->geo[i]->locality       = strdup(json_get_string(Json_Geo, "locality")); 
      _Geos->geo[i]->timezone       = strdup(json_get_string(Json_Geo, "timezone")); 

      _Geos->geo[i]->street_number  = json_get_int(Json_Geo, "street_number");

      memcpy(_Geos->geo[i]->timezone_gmt, json_get_string(Json_Geo, "timezone_gmt"), 6);
      _Geos->geo[i]->timezone_gmt[6] = '\0';
      memcpy(_Geos->geo[i]->country_code, json_get_string(Json_Geo, "country_code"), 2);
      _Geos->geo[i]->country_code[2] = '\0';
