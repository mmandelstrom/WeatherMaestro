...
int meteo_init_ptr(Meteo **_M_Ptr, Meteo_Geo **_MG_Ptr, Meteo_Current **_MW_Ptr,
                   Meteo_Hourly **_MF_Ptr);
...
void meteo_dispose_ptr(Meteo **_M_Ptr, Meteo_Geo **_MG_Ptr,
                       Meteo_Current **_MW_Ptr, Meteo_Hourly **_MF_Ptr);

/* ----------------------------------------------------------------- */

int meteo_get_geo(Meteo **_M_Ptr, float _lat, float _lon, int _count,
                  on_ext_api_finish _on_finish, void *_context) {
  Meteo_Geo *Geo;
...
int meteo_get_weather_current(Meteo **_M_Ptr, float _lat, float _lon,
                              on_ext_api_finish _on_finish, void *_context) {
...
int meteo_get_weather_hourly(Meteo **_M_Ptr, float _lat, float _lon,
                             on_ext_api_finish _on_finish, void *_context) {
  Meteo_Hourly *Hourly;
...
int meteo_init_ptr(Meteo **_M_Ptr, Meteo_Geo **_MG_Ptr, Meteo_Current **_MW_Ptr,
  ...
  if (_MW_Ptr != NULL) {
    *_MW_Ptr = calloc(1, sizeof(Meteo_Current));
    ...
    (*_MW_Ptr)->values =
        realloc((*_MW_Ptr)->values, sizeof(Meteo_Weather_Values));
...
int meteo_get_weather_json(Meteo *_M, float _lat, float _lon, bool _forecast) {
  ...
  if (_forecast) {
    int url_len = snprintf(url, 512, METEO_BASE_URL, _lat, _lon,
  ...
  if (http_client_initiate(c, (const char *)url, method,
                           meteo_on_http_client_finish, _M,
                           &_M->http_response) != SUCCESS) {
    return ERR_INTERNAL;
  }
  return SUCCESS;
}
...
int meteo_parse_json_weather_current(Meteo_Current *_MW, const char *_json) {
  ...
  cJSON *Cur_Weather_Units =
      cJSON_GetObjectItemCaseSensitive(Json_Root, "current_units");
  ...
  _MW->values->wind_direction_10m =
      json_get_int(Cur_Weather, "wind_direction_10m");
  ...
  /* Heap allocations */
  _MW->timezone_abbreviation =
      strdup(json_get_string(Json_Root, "timezone_abbreviation"));
  _MW->temperature_2m_unit =
      strdup(json_get_string(Cur_Weather_Units, "temperature_2m"));
  _MW->elevation_unit = strdup(json_get_string(Cur_Weather_Units, "elevation"));
  _MW->precipitation_unit =
      strdup(json_get_string(Cur_Weather_Units, "precipitation"));
  _MW->wind_speed_10m_unit =
      strdup(json_get_string(Cur_Weather_Units, "wind_speed_10m"));
  _MW->wind_direction_10m_unit =
      strdup(json_get_string(Cur_Weather_Units, "wind_direction_10m"));
  if (_MW->timezone_abbreviation == NULL || _MW->temperature_2m_unit == NULL ||
      _MW->elevation_unit == NULL || _MW->precipitation_unit == NULL ||
...
int meteo_parse_json_weather_hourly(Meteo_Hourly *_MF, const char *_json) {
  ...
  cJSON *Hourly_Weather_Units =
      cJSON_GetObjectItemCaseSensitive(Json_Root, "hourly_units");
  ...
  _MF->values =
      realloc(_MF->values, ((size_t)count * sizeof(Meteo_Weather_Values)));
  if (_MF->values == NULL) {
    perror("realloc");
    cJSON_Delete(Json_Root);
    return ERR_NO_MEMORY;
  ...
  cJSON *Temperature =
      cJSON_GetObjectItemCaseSensitive(Hourly_Weather, "temperature_2m");
  cJSON *Windspeed =
      cJSON_GetObjectItemCaseSensitive(Hourly_Weather, "wind_speed_10m");
  cJSON *Winddirection =
      cJSON_GetObjectItemCaseSensitive(Hourly_Weather, "wind_direction_10m");
  cJSON *Precipitation =
      cJSON_GetObjectItemCaseSensitive(Hourly_Weather, "precipitation");
  cJSON *Weathercode =
      cJSON_GetObjectItemCaseSensitive(Hourly_Weather, "weather_code");
  cJSON *Isday = cJSON_GetObjectItemCaseSensitive(Hourly_Weather, "is_day");

  if (Temperature == NULL || Windspeed == NULL || Winddirection == NULL ||
      Precipitation == NULL || Weathercode == NULL || Isday == NULL) {
  ...
  _MF->timezone_abbreviation =
      strdup(json_get_string(Json_Root, "timezone_abbreviation"));
  _MF->temperature_2m_unit =
      strdup(json_get_string(Hourly_Weather_Units, "temperature_2m"));
  _MF->elevation_unit =
      strdup(json_get_string(Hourly_Weather_Units, "elevation"));
  _MF->precipitation_unit =
      strdup(json_get_string(Hourly_Weather_Units, "precipitation"));
  _MF->wind_speed_10m_unit =
      strdup(json_get_string(Hourly_Weather_Units, "wind_speed_10m"));
  _MF->wind_direction_10m_unit =
      strdup(json_get_string(Hourly_Weather_Units, "wind_direction_10m"));

  /* Heap allocations */
  if (_MF->timezone_abbreviation == NULL || _MF->temperature_2m_unit == NULL ||
      _MF->elevation_unit == NULL || _MF->precipitation_unit == NULL ||
  ...
  for (int i = 0; i < count; i++) {
    ...
    memcpy(_MF->values[i].timestamp, json_get_string(Hourly_Weather, "time"),
           16);

    _MF->values[i].timestamp[16] = '\0';

    _MF->values[i].precipitation =
        json_get_double(Hourly_Weather, "precipitation");
    _MF->values[i].temperature_2m =
        json_get_double(Hourly_Weather, "temperature_2m");
    _MF->values[i].wind_speed_10m =
        json_get_double(Hourly_Weather, "wind_speed_10m");
    _MF->values[i].wind_direction_10m =
        json_get_int(Hourly_Weather, "wind_direction_10m");
...
void meteo_dispose_ptr(Meteo **_M_Ptr, Meteo_Geo **_MG_Ptr,
  ...
  if (_M_Ptr != NULL) {
    if (*_M_Ptr != NULL) {
      if((*_M_Ptr)->http_response != NULL) {
        free((*_M_Ptr)->http_response);
        (*_M_Ptr)->http_response = NULL;
      }
      if ((*_M_Ptr)->geo != NULL)
