...
int nominatim_parse_geocode_json(Nominatim_Geo **_NOM_Geo_Ptr, int *_geo_count,
                                 const char *_json);
...
int nominatim_get_geo_by_query(Nominatim_Geo **_NOM_Geo, int *_geo_count,
                               const char *_query) {
  ...
  int result =
      nominatim_parse_geocode_json(_NOM_Geo, _geo_count, nominatim_json);
  if (result != 0) {
...
int nominatim_get_geo_by_coords(Nominatim_Geo *_NOM_Geo, float _lat,
                                float _lon) {
...
const char *nominatim_get_geocode_json_by_query(const char *_query) {
  ...
  int url_len = snprintf(url, 512, NOMINATIM_GEOCODE_QUERY, _query);
  url[url_len] = '\0';
...
int nominatim_parse_geocode_json(Nominatim_Geo **_NOM_Geo_Ptr, int *_geo_count,
  ...
  for (int i = 0; i < *_geo_count; i++) {
    ...
    cJSON *Json_Address =
        cJSON_GetObjectItemCaseSensitive(Json_Place, "address");
    if (Json_Place == NULL) {
    ...
    memcpy((*_NOM_Geo_Ptr)[i].country_code,
           json_get_string(Json_Address, "country_code"), 2);
    ...
    if (parse_string_to_double(lat, &(*_NOM_Geo_Ptr)[i].lat) != 0 ||
        parse_string_to_double(lon, &(*_NOM_Geo_Ptr)[i].lon) != 0) {
    ...
    printf("NOM_Geo[%i] lat: %lf lon: %lf\n", i, (*_NOM_Geo_Ptr)[i].lat,
           (*_NOM_Geo_Ptr)[i].lon);

    /* Heap allocations */
    (*_NOM_Geo_Ptr)[i].country =
        strdup(json_get_string(Json_Address, "country"));
    ...
    (*_NOM_Geo_Ptr)[i].postcode =
        strdup(json_get_string(Json_Address, "postcode"));
    (*_NOM_Geo_Ptr)[i].road = strdup(json_get_string(Json_Address, "road"));
    (*_NOM_Geo_Ptr)[i].house_number =
        strdup(json_get_string(Json_Address, "house_number"));

    if ((*_NOM_Geo_Ptr)[i].country == NULL ||
        (*_NOM_Geo_Ptr)[i].county == NULL || (*_NOM_Geo_Ptr)[i].city == NULL ||
        (*_NOM_Geo_Ptr)[i].postcode == NULL ||
        (*_NOM_Geo_Ptr)[i].road == NULL ||
        (*_NOM_Geo_Ptr)[i].house_number == NULL) {
      fprintf(stderr,
              "One or more strings couldn't be parsed from nominatim json\n");
      cJSON_Delete(Json_Root);
      return -6;
    }
  }
...
http://osm.org/copyright", "osm_type": "node", "osm_id": 1298718355, "lat":
"59.2927684", "lon": "18.0812270", "class": "place", "type": "house",
    "place_rank": 30,
    "importance": 0.0000840184634164495,
    "addresstype": "place",
    "name": "",
    "display_name": "61, Arenavägen, Johanneshov, Enskede-Årsta-Vantörs
stadsdelsområde, Stockholm, Stockholms kommun, Stockholms län, 121 62, Sverige",
    "address": {
      "house_number": "61",
      "road": "Arenavägen",
      "suburb": "Johanneshov",
...
[
  {
    "place_id": 154042173,
    "licence": "Data © OpenStreetMap contributors, ODbL 1.0.
http://osm.org/copyright", "osm_type": "node", "osm_id": 25735371, "lat":
"59.8586126", "lon": "17.6387436", "class": "place", "type": "city",
    "place_rank": 16,
