...
int weather_api_init_ptr(Weather_API **_Weather_API_Ptr,
                         HTTP_Request *_HTTP_Req, HTTP_Response *_HTTP_Res,
                         on_api_finish _on_finish, void *_context) {
  ...
  *_Weather_API_Ptr = malloc(sizeof(Weather_API));
  ...
  (*_Weather_API_Ptr)->task =
      scheduler_create_task((*_Weather_API_Ptr), weather_api_taskwork);
...
int weather_api_handle_endpoint_geo_get(Weather_API *_API) {
  ...
  linked_list_foreach(_API->http_request->params, node) {
    ...
    if (Param->key != NULL && Param->value != NULL) {
      if ((strcmp(Param->key, "q") == 0 || strcmp(Param->key, "city") == 0) &&
          query_found < 1) {
  ...
  if (query_found > 0) {
    ...
    if (geo_parser_get_geo_by_query(_API->geos, query,
                                    &_API->http_response->body) != 0) {
  ...
  printf("Here we set 200\n");
  _API->http_response->status_code = 200;
  _API->state = WEATHER_API_RESPONDING;
  return SUCCESS;
}
...
int weather_api_handle_endpoint_weather_get(Weather_API *_API) {
  ...
  linked_list_foreach(_API->http_request->params, node) {
    ...
    if (Param->key != NULL && Param->value != NULL) {
      ...
      if (strcmp(Param->key, "longitude") == 0 ||
          strcmp(Param->key, "lon") == 0)
        lon_found += geo_parser_lat_lon(Param->value, &lon);

      /* if (strcmp(Param->key, "city") == 0 || strcmp(Param->key, "cityname")
        == 0) city_found += weather_parser_city() */
  ...
  if (lat_found > 0 &&
      lon_found > 0) // Could add an if else for city->name != NULL and let
                     // parser find coords for that city then
  {
    if (weather_parser_init_ptr(&_API->weather_parser, _API,
                                weather_api_on_parser_finish, lat, lon,
                                false) != 0) {
      perror("weather_parser_init_ptr");
      _API->http_response->status_code = 500;
      return ERR_INTERNAL;
...
WeatherApiState weather_api_worktask_validate_endpoint(Weather_API *_API) {
  ...
  int res = _API->endpoint_func(_API);
  ...
  char endpoint_path[128];
  strcpy(endpoint_path, API_ENDPOINT_ROOT);
  strcat(endpoint_path, "/geo");
  printf("request_path: %s\n", _API->http_request->path);
  printf("endpoint_path: %s\n", endpoint_path);
  if (strcmp(_API->http_request->path, endpoint_path) == 0) {
    printf("Setting endpoint func\n");
    return WEATHER_API_RESPONDING;
  }
