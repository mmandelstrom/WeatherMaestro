...
{
  if (_Weather_Req_Ptr == NULL)
    return -1;

  *_Weather_Req_Ptr = malloc(sizeof(Weather_Request));
  if (*_Weather_Req_Ptr == NULL)
  {
    perror("malloc");
    return -2;
  ...
  memset(*_Weather_Req_Ptr, 0, sizeof(Weather_Request));

  (*_Weather_Req_Ptr)->http_request = _HTTP_Req;
...
/** Takes a float string and tries to convert it to float
...
int weather_api_handle_endpoint_weather_get(Weather_Request* _Request)
{
  if (_Request->http_request->params_count < 2)
    return -1;
  ...
  _Request->city = New_City;

  int lat_found = 0;
  int lon_found = 0;
  for (int i = 0; i < _Request->http_request->params_count; i++)
  {
    yuarel_param* param = _Request->http_request->params;
    if (param[i].key != NULL && param[i].val != NULL)
    {
      if (strcmp(param[i].key, "latitude") == 0)
        lat_found += weather_utils_parse_lat_lon(param[i].val, &_Request->city->lat);

      if (strcmp(param[i].key, "longitude") == 0)
        lon_found += weather_utils_parse_lat_lon(param[i].val, &_Request->city->lon);
  ...
  {
    /* _Request->http_request->body = meteo_get_weather(_Request->city->lat, _Request->city->lon); */
...
{
  WeatherAPIEndpoint endpoint = weather_api_get_endpoint(_Request->http_request->path);
  ...
  switch(endpoint)
  {
    case ENDPOINT_WEATHER_GET:
    {
      ...
      result = weather_api_handle_endpoint_weather_get(_Request);
